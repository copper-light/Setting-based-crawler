...
public class Scraper {
	...
	public static Document GetDocument(Work urlInfo) throws JSONException, org.openqa.selenium.TimeoutException,
			WebDriverException, NoSuchAlgorithmException, KeyManagementException, IOException {
		if (TYPE.contentEquals(Config.CRAWLING_TYPE_SCENARIO_DYNAMIC)) {
	 ...
	 * @return Document
	 */
	private static Document useJsoup(Work urlInfo)
	...
	private static Document useSelenium(Work work) throws JSONException, TimeoutException, WebDriverException {
		Document ret = null;
		...
		if (action != null) {
			...
			if (selector != null) {
				...
				if (wes.size() == 1) {
					...
					// urlInfo.getLoadCheckSelectors().size() > 0){
	...
	private static boolean connectSelenium() {
		try {
